 ...
 */
package com.nepheletech.sslutils;
...
public class ExtensionsUtils {
  ...
  public static boolean isSubtreeOf(String parentOid, String oid) {
    ...
    if (parentParts.length >= oidParts.length) {
      ...
      for (int i = 0; i < parentParts.length; i++) {
        if (!parentParts[i].equals(oidParts[i])) { return false; }
      }
      return true;
    }
  }
  ...
  /**
   * Given a Java X509Certificate object, return a list of maps representing all
   * the X509 extensions embedded in the certificate. If no extensions exist on
   * the certificate, then null is returned.
   *
  ...
  /**
   * Given a Java X509CRL object, return a list of maps representing all the X509
   * extensions embedded in the CRL. If no extensions exist on the CRL, then null
   * is returned.
   *
  ...
  /**
   * Given a Bouncy Castle CSR object, return a list of maps representing all the
   * X509 extensions embedded in the CSR. If no extensions exist on the CSR, then
   * null is returned.
   *
  ...
  /**
   * Given a Java certificate, get a map containing the value and criticality of
   * the extensions described by the given OID. If the OID is not found in the
   * certificate then null is returned.
   *
  ...
  /**
   * Given a Java X509CRL object, get a map containing the value and criticality
   * of the extensions described by the given OID. If the OID is not found in the
   * CRL, then null is returned. If no extensions exist on the CRL, then null is
   * returned.
   *
   * @param crl The X509 CRL object.
   * @param oid The OID of the extension to be found.
  ...
  /**
   * Given a Bouncy Castle CSR, get a map describing an extension value and its
   * criticality from its OID. If the extension is not found then null is
   * returned.
   *
  ...
  /**
   * Given a list of maps describing extensions, return a map containing the
   * extensions described by the provided OID. Returns null if the OID doesn't
   ...
   * @return The map describing the found extension, null if the oid doesn't
   *         exist.
  ...
  public static Map<String, Object> getExtension(List<Map<String, Object>> extList,
    ...
    for (Map<String, Object> ext : extList) {
      if (ext.get("oid").equals(oid)) { return ext; }
    }
  ...
  /**
   * Given an extensions container and an OID, extract the value and criticality
   * flag and return the values in a map. If the extension is not found then null
   * is returned.
   *
  ...
  /**
   * Find the X509 Extensions from CSR object. If no extensions attribute is found
  ...
  static Extensions getExtensionsFromCSR(PKCS10CertificationRequest csr) {
    ...
    for (Attribute attr : attrs) {
      ...
      if (extsAsn1 != null) {
        ...
        } else if (extObj instanceof DERSequence) { return Extensions.getInstance(extObj); }
      }
    }
  ...
  /**
   * Provided a map which describes an X509 extension, parse it into a Bouncy
   * Castle Extension object.
   *
  ...
  /**
   * Get a Bouncy Castle Extensions container from a Java X509 certificate object.
   * If no extensions are found then null is returned.
   *
   * @param cert The Java X509 certificate object.
  ...
  /**
   * Get a Bouncy Castle Extensions container from a Java X509 CRL object. If no
   * extensions are found then null is returned.
   *
   * @param crl The Java X509 CRL object.
   * @return A Bouncy Castle Extensions container object extracted from the CRL.
   * @throws CRLException
   * @throws IOException
   */
  ...
  /**
   * Given an Extensions container, an OID, and a critical flag, create a map of
   * this extension's data with the following keys:
   *
   * - "oid" : The OID of the extensions - "value" : A String, or list of Strings
   * of this OID's data - "critical" : A bool set to true if this extensions is
   * critical, false if it isn't.
   *
   * If the given OID doesn't exist in the extensions container then null is
   * returned.
   *
   * @param exts     A Bouncy Castle Extensions container containing the provided
   *                 OID
   ...
   * @return A map representing the extension with the given OID which exists in
   *         the provided Extensions container.
   * @throws IOException
   */
  private static Map<String, Object> makeExtensionMap(Extensions exts,
      ASN1ObjectIdentifier oid,
   ...
   * which represents the data it contains accord to its OID. I've searched all
   * over the BouncyCastle API and I can't seem to find this mapping defined
   * anywhere, so I've created it here.
   *
   * @param oid  The extension OID.
   * @param data The binary data value of the extension with the given OID.
   * @return An ASN1Object which contains the data described by the provided OID.
   * @throws IOException
   */
  ...
  /**
   * Convert a Bouncy Castle ASN1Object into a Java data structure, which will
   * generally be in the form of a string, map, list or combination thereof. If
   * this method can't determine a method of converting the ASN1 object then the
   * raw byte array is returned.
   *
  ...
  private static BasicConstraints mapToBasicConstraints(Map<String, Object> bcMap) {
    ...
    if (isCa == null) { throw new IllegalArgumentException(
  ...
  private static AuthorityKeyIdentifier mapToAuthorityKeyIdentifier(
    ...
    if (pubKey != null) {
      SubjectPublicKeyInfo authPubKeyInfo = SubjectPublicKeyInfo.getInstance(
          pubKey.getEncoded());

      DigestCalculator digCalc = new BcDigestCalculatorProvider().get(
  ...
  private static Integer getGnTagFromName(String name) {
    for (int i = 0; i < generalNameTags.size(); i++) {
      if (generalNameTags.get(i).equalsIgnoreCase(name)) { return i; }
    }
  ...
  /**
   * Convert the value of an IP address which is encoded in an ASN1OctetString to
   * a string.
   *
  ...
  /**
   * Convert a Bouncy Castle GeneralNames object into a Java map where the key is
   * the type of name defined, and the value is a list of names of that type.
   *
   * @param names The GeneralNames object to be parsed.
   * @return A list of the names contained in each GeneralName in the GeneralNames
   *         data structure.
   * @throws IOException
